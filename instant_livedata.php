			...
			if(pg_num_rows($result) == 0){
				...
				if(pg_num_rows($result) > 0){
					...
					if ((!($livedataspeedup = apc_fetch("livedata.json - share count from $tempid"))) || ($nocache)){
						...
						$sql = "select sum(pow(2,32)) as instcount, max(id) as latest_id from shares where server=$serverid and our_result=true and id > $maxid";
						$result = pg_exec($link, $sql); $row = pg_fetch_array($result, 0);
						$roundshares += $row["instcount"];
						$maxid = $row["latest_id"];
						if (($roundshares > 0) && ($maxid > $tempid)) {
				...
				if($cppsrbjsondec = apc_fetch('cppsrb_json_inst')) {
					...
					if (filemtime("$pooldatadir/$serverid/cppsrb.json") > (time()-600)) {
						...
						$sql = "select (sum(accepted_shares)*pow(2,32))/1350 as avghash,sum(accepted_shares) as share_total from $psqlschema.stats_shareagg where server=$serverid and time > to_timestamp($sql2res) at time zone 'UTC' -'1350 seconds'::interval";
						$result = pg_exec($link, $sql); $row = pg_fetch_array($result, 0);
